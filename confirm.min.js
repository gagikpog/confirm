...
config=__assign({theme:'white',modal:!0,templateId:''},config);var contentForm=document.createElement('form');contentForm.addEventListener("submit",function(event){event.preventDefault()},!0);var buttonsContainer=document.querySelector(dialogId+" .messageBox-buttons");while(buttonsContainer.hasChildNodes()){buttonsContainer.removeChild(buttonsContainer.lastChild)}
config.buttons.forEach(function(button){if(isNeedUseDefaultButtons&&!(button.id in config)){return}
var btn=document.createElement('button');btn.classList.add('btn','mbButton');btn.textContent=button.title;btn.id=button.id;btn.onclick=function(){dialog.style.display='none';var formData=new FormData(contentForm);var result={};for(var _i=0,_a=formData.entries();_i<_a.length;_i++){var pair=_a[_i];if(pair[0]){result[pair[0]]=pair[1]}}
var wait=res({button:btn.id,formData:result});if(wait&&typeof(wait.then)==='function'){wait.then(function(){dialog.remove()})}
else{dialog.remove()}};btn.style.backgroundColor=button.backgroundColor;btn.style.color=button.color;btn.style.order=button.order;buttonsContainer.appendChild(btn)});dialog.classList=[];dialog.classList.add('confirm',config.theme);dialog.style.display='flex';if(config.modal){dialog.classList.add('modal')}
